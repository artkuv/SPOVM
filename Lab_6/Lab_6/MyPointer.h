...
/// <summary>
/// MyPointer class serves to ease the work with Manager class, by providing
...
private:
	/// <summary>
	/// This class will serve as a counter for references, in order to provide garbage collection in MyPointer destructor.
	/// </summary>
...
public:
	...
	int copyData(Type* source, int source_size)
	{
		if (this->memory_manager_ == nullptr)
			throw AbsentException("Memory manger is ansent!");

		int min_size = (*(this->size_) > source_size) ? source_size : *(this->size_);

		int i = 0;
		std::list<ManagerInterface::MemoryBlockData*>::iterator iterator;
	...
	void setMemoryManager(ManagerInterface* memory_manager)
	{
		if (this->memory_manager_ != nullptr)
			this->freeMemory();
...
public:
	MyPointer() : memory_manager_(nullptr), stub_(0)
	{
		this->connected_memory_ = new std::list<ManagerInterface::MemoryBlockData*>;
	...
	MyPointer(ManagerInterface* memory_manager) : memory_manager_(memory_manager), stub_(0)
	{
		this->connected_memory_ = new std::list<ManagerInterface::MemoryBlockData*>;
		// Create a new reference 
		this->references_ = new ReferenceCounter();
	...
	MyPointer(const MyPointer<Type>& other) :
...
private:
	ManagerInterface * memory_manager_;
	ReferenceCounter* references_;

	std::list<ManagerInterface::MemoryBlockData*>* connected_memory_;
	int* size_;
