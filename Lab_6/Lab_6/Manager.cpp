...
const int Manager::MAX_DATA_SIZE_IN_BYTES = 10240;
const string Manager::disk_file_name_ = "fbuff";
...
/// </returns>
list<ManagerInterface::MemoryBlockData*> Manager::allocate(size_t size_in_bytes, size_t type_size)
{
	// checking if there is enough memory space left
	if ((this->distributed_space_ + size_in_bytes) > this->MAX_DATA_SIZE_IN_BYTES)
		throw AllocationException("Not enough memory space!");

	// a list of memory blocks, which will be returbed to a calling MyPointer
...
/// A list of blocks, which were previously allocated by Manager
/// </param>
void Manager::free(list<ManagerInterface::MemoryBlockData*> blocks_to_delete)
{
...
/// </param>
void Manager::eraseBlock(MemoryBlockData* block)
{
...
/// <summary>
/// Defragmnetates free memory by flushing all the data on disk, and then restoring it.
...
/// </summary>
void Manager::DefragmentateFreeMemory()
{
