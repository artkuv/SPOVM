...
vector<string> AIO::GetFilesFromSource(const string& source_folder)
{
...
int AIO::ConcatFiles(std::string source_folder_path, std::string output_file_path)
{
	...
	for (auto &file_name : this->source_file_names_)
	{
		...
		else
		{
			...
			this->read_files_data_.back()->data = new char[this->read_files_data_.back()->size]; //буфер
			...
			this->thread_data_vector_.back()->overlapped.hEvent = this->read_events_.back(); //показываем события для переведения в сиг сост

			this->reading_threads_.push_back(CreateThread(NULL, NULL, Read, reinterpret_cast<LPVOID>(this->thread_data_vector_.back()), NULL, NULL)); //поток чтения
	...
	this->writing_thread_ = CreateThread(NULL, NULL, Write, static_cast<LPVOID>(this), NULL, NULL); //поток записи
...
DWORD WINAPI AIO::Write(LPVOID param)
{
	AIO* caller_instance = reinterpret_cast<AIO*>(param);
...
DWORD WINAPI AIO::Read(LPVOID param)
{
